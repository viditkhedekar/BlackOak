...
- **R4 — Autonomous execution engine — ✅ BUILT (2026-07-26).** `domain/decision.py` (`plan_cycle` — the shared brain; backtester refactored to use it, so backtest/live parity is by construction). Migration 0008 (positions/orders/executions/`position_theses`/`trade_decisions`/`portfolio_snapshots`). `BrokerClient` port + `domain/broker.py` DTOs; `alpaca_broker.py` (paper-URL hard-asserted, order caps, client_order_id idempotency) + `FakeBroker`. `services/execution.py` (idempotent placement, deterministic client_order_id), `reconciliation.py` (broker-truth sync), `decision_engine.py` (reconcile → snapshot → fuses → plan_cycle → **journal every decision with evidence BEFORE acting** → execute → snapshot). 30-min `intraday_cycle` job (worker now runs 4 jobs). **Gate met:** 100 tests incl. execution parity (backtest & live honor identical plan) + client_order_id idempotency; live cycle verified vs real DB with FakeBroker (25 symbols each journaled with a reason before acting). No real orders placed.
- **R5 — Dashboard — ✅ BUILT (2026-07-26).** API: `schemas/dashboard.py` + routers for /strategy/rankings, /strategy/regime, /strategy/decisions, /portfolio, /performance, /backtests(+/{id}), /system/health. Frontend (React Query 15s polling): Command Center (regime + stats + live positions/decision feed), **Decision Journal** (centerpiece — filterable buy/sell/hold/skip/blocked with expandable evidence), Rankings (strategy_scores heat table + regime weights), Backtest Lab (ECharts equity curve + metrics vs SPY/RSP + trades), System Health (job runs + feed freshness); RegimeBadge; nav rebuilt. **Gate met:** browser-verified all pages render live data (Journal = 25 journaled decisions with reasons; Rankings JPM #1 under risk_on weights; System Health full pipeline history); tsc+lint+build green. Remaining (deferred within R5): Signal Explorer per-stock indicator panes, Portfolio & Risk correlation/VaR page.
